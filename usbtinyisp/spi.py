...
usage = """Available commands: (everything in hex)
	t                          - echo test
	...
	x <c>                      - issue a single SPI command
	...
	Example for sending one spi command:
		spi.py U 1                # powerup, otherwise 'No response'
		spi.py x 60               # sends 0x60 and displays response in hex
	"""
...
if cmd == 't':
	...
	r = dev.control_in(USBTINY_SPI1, byte, 0, 1)
	if len(r) == 1:
		print "%02x" % ord(r[0])
	else:
		print "No response"
